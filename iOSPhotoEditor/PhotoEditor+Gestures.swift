...
extension PhotoEditorViewController: UIGestureRecognizerDelegate {
  ...
  @objc func pinchGesture(_ recognizer: UIPinchGestureRecognizer) {
    if let view = recognizer.view {
      ...
      if view is UITextView {
        ...
        let size = CGSize(width: UIScreen.main.bounds.size.width, height: .greatestFiniteMagnitude)

        if textView.font!.pointSize * recognizer.scale < 90 {
          textView.font = textView.font?.withSize(textView.font!.pointSize * recognizer.scale)
  ...
  func moveView(view: UIView, recognizer: UIPanGestureRecognizer) {
    ...
    view.center = CGPoint(
      x: view.center.x + recognizer.translation(in: canvasImageView).x,
      y: view.center.y + recognizer.translation(in: canvasImageView).y
    )

    recognizer.setTranslation(.zero, in: canvasImageView)
